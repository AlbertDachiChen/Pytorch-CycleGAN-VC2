...
class trainingDataset_Paired(Dataset):
    def __init__(self, datasetA, datasetB, max_frames=512):
        self.datasetA = datasetA
        self.datasetB = datasetB
    ...
    def __getitem__(self, index):
        ...
        data_A = dataset_A[index]
        data_B = dataset_B[index]

        frames_A_total = data_A.shape[1]
        frames_B_total = data_B.shape[1]
        min_frames = min(frames_A_total, frames_B_total)
        if min_frames <= self.max_frames:
            return np.array(data_A), np.array(data_B)
        
        else:
            start = np.random.randint(min_frames - self.max_frames + 1)
            end = start + self.max_frames
            return np.array(data_A[:, start:end]), np.array(data_B[:, start:end])
